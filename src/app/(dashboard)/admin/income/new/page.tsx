...
export default function NewIncomePage() {
  ...
  const list = useQuery({
    ...
    queryFn: () => api<{ income: IncomeRow[] }>('/api/admin/income'),
  });

  const create = useMutation({
    mutationFn: (values: FormValues) =>
      api<{ income: IncomeRow }>('/api/admin/income', {
        method: 'POST',
        body: JSON.stringify({
          date: values.date,
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['income-today'] });
      reset({
        date: format(new Date(), 'yyyy-MM-dd'),
        category: '',
        amount: 0,
  ...
  const totalToday = (list.data?.income ?? []).reduce(
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-slate-900">Record manual income</h1>
          <p className="text-sm text-slate-500">Income not tied to an invoice (e.g. external lab work).</p>
        </div>
        <Link
          href="/admin"
      ...
      <Card>
        <CardHeader title="New income entry" />
        <CardBody>
          <form
          ...
          >
            ...
            <Field label="Category" required error={errors.category?.message}>
              <Input
                placeholder="e.g. External lab, Consultation"
                {...register('category')}
              />
            </Field>
            ...
            <div className="flex justify-end gap-2 sm:col-span-2">
              ...
              <Button type="submit" disabled={create.isPending}>
                {create.isPending ? 'Saving…' : 'Save income'}
              </Button>
            </div>
          </form>
        </CardBody>
      ...
      <Card>
        <CardHeader
          title="Today"
          subtitle={`${(list.data?.income ?? []).length} entry(ies) · Total ${formatMoney(totalToday.toFixed(2))}`}
          action={
            <button
        ...
        <CardBody className="p-0">
          ...
          ) : (list.data?.income ?? []).length === 0 ? (
            <div className="px-5 py-6">
              <EmptyState title="No income entries today" />
            </div>
          ) : (
